...
describe("qiaobianguniang", () => {
  ...
  it("follows the official 阿波 score patterns with marked hands", () => {
    ...
    // 尾奏弱唱段全休止（第 54–57 小节），第 58 小节单音收尾
    for (const bar of qiaobianguniangBars.slice(53, 57)) {
  ...
  it("labels sections at their first bars and aligns all 24 lyric sentences", () => {
    ...
    expect(sectionAt(6)).toBe("主歌一");
    expect(sectionAt(14)).toBe("主歌二");
    expect(sectionAt(22)).toBe("副歌");
    expect(sectionAt(34)).toBe("主歌三");
    expect(sectionAt(50)).toBe("副歌");
    expect(sectionAt(54)).toBe("尾奏");
    expect(qiaobianguniangBars[1].section).toBeUndefined();

    const lyricBars = qiaobianguniangBars.filter((bar) => bar.lyric);
    // 26 句歌词，每句 2 小节，覆盖第 6–57 小节的人声部分
    expect(lyricBars).toHaveLength(26);
    for (const bar of lyricBars) expect(bar.lyricSpan).toBe(2);
    expect(lyricBars[0].number).toBe(6);
    expect(lyricBars.at(-1)!.number).toBe(56);

    const lyricAt = (barNumber: number) => qiaobianguniangBars[barNumber - 1].lyric;
    expect(lyricAt(6)).toBe("暖阳下 我迎芬芳");
    expect(lyricAt(8)).toBe("是谁家的姑娘");
    expect(lyricAt(28)).toBe("我把你放心上 刻在了我心膛");
    expect(lyricAt(34)).toBe("暖阳下 的桥头旁");
    expect(lyricAt(38)).toBe("她有着长长的乌黑发");
    expect(lyricAt(48)).toBe("一个人在流浪");
    expect(lyricAt(56)).toContain("不想让你流浪");
  });
